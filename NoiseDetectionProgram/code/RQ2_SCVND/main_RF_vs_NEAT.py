...
#     read_type_IL = "unfilteredIL";
    ...
    # 使用的分类器名字
    list_names_classifiers = ["Random Forest"]#CNDC文献所使用的5个分类器
...
#             i_projectName='commons-collections';
            ...
            for i_num in range(0,len(fileList)):
                ...
                if i_num == 0:
                    ...
                    len_instances = len(df_result);
                    labels_predicted = np.zeros((len_instances), dtype=int);
                    # psx大于等于0.5
                    idx_predicted_1 = np.where(array_psx_allClassifiers[:, 1] > 0.5)[0];
    ...
    #                     features_only = copy.copy(features_label);
                    ...
                    labels_predicted = array_labels.reshape(-1)
                # 和带噪音的s标签比，挑出预测错误的标签，预测错误的标签被认为是类噪音
                label_errors_bool = ~(labels_predicted == df_test['bug'].values);
                errors_idx_all = np.arange(len(label_errors_bool))[label_errors_bool];
                # 根据预测概率生成dataframe
...
#                 FUNCTION_CalculatePredictionResultALLMethod();
                ...
                list_mislabels_RF = [errors_idx_all,errors_idx_1,errors_idx_0];
                #---end---#
                ...
                df_result_mislabel.loc[errors_idx_all,['predictedMislabel']] = 1;
                #记录真实误标签的结果
                ...
                if i_num != 0:
                    ...
                    list_mislabels_HR2 = [];
                    list_mislabels_NEAT = [];
                    list_mislabels_HR1_NEAT = [];
                    list_mislabels_HR2_NEAT = [];
                    list_mislabels_NEAT_NEAT = [];
...
#                     df_current_labels_n_low = df_train_original_known
                    ...
                    df_result_mislabel_NEAT = df_dataset_labels;
                    df_result_mislabel_NEAT['predictedMislabel'] = 0;
                    df_result_mislabel_NEAT.loc[list_mislabels_NEAT[0],['predictedMislabel']] = 1;
                    df_result_mislabel_NEAT = pd.merge(df_result_mislabel_NEAT, df_mislabels_groundTruth, how='left', on='instances');
                    df_result_mislabel_NEAT['realMislabel'].fillna(0, inplace=True);
                    df_result_mislabel_NEAT = df_result_mislabel_NEAT[['instances',"predictedMislabel","realMislabel"]];
                    df_groundTruth_part_NEAT = df_result_mislabel_NEAT[(df_result_mislabel_NEAT['predictedMislabel'] == 1) & (df_result_mislabel_NEAT['realMislabel'] == 1)];
                    df_groundTruth_part_NEAT = df_groundTruth_part_NEAT[['instances',"realMislabel"]];
                    
                    if i_num == 1:
                        list_mislabels_HR1_NEAT = Class_NEAT.list_mislabels_HR1;
                        list_mislabels_HR2_NEAT = Class_NEAT.list_mislabels_HR2;
                        list_mislabels_NEAT_NEAT = Class_NEAT.list_mislabels_NEAT;
                    else:
                        df_current_labels_g_low = df_groundTruth_part_NEAT;
                        Class_NEAT.FUNCTION_HeuristicRules(df_current_labels_n_low, df_current_labels_g_low, df_test);
                        list_mislabels_HR1_NEAT = Class_NEAT.list_mislabels_HR1;
                        list_mislabels_HR2_NEAT = Class_NEAT.list_mislabels_HR2;
                        list_mislabels_NEAT_NEAT = Class_NEAT.list_mislabels_NEAT;
    ...
    #                     i_type_mislabels = 2;#调试用，1："(buggy)mislabels"
                        ...
                        if index_realMislabels_currentType.size!=0:
                            ...
                            #baseline RF
                            errors_idx = list_mislabels_RF[i_type_mislabels];
                            list_result = FUNCTION_calculateIndicators_NEAT("Random Forest",errors_idx,index_realMislabels_currentType,originIndex_currentType);
                            ...
                            #baseline: NEAT-II模型
                            errors_idx = list_mislabels_NEAT_NEAT[i_type_mislabels];
                            list_result = FUNCTION_calculateIndicators_NEAT("NEAT-II",errors_idx,index_realMislabels_currentType,originIndex_currentType);
                            list_indicators_allBaselines.extend(list_result);
